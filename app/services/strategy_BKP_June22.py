...
def check_entry_signal(
    ...
    # Dual-timeframe EMA alignment gate: EMA(fast) must agree with EMA(slow)
    # on BOTH 15-min and 1-min bars.  15-min catches slow trend deterioration;
    # 1-min catches intrabar crossovers (e.g. stock already recovering toward
    # VWAP before the 15-min bar closes).  One timeframe disagreeing = no entry.
    direction_prelim = "CALL" if trend == "bullish" else "PUT"
    if not check_ema_alignment(bars_15m, direction_prelim, bars_1m):
        return None
    ...
    min_clearance = settings.vwap_min_clearance_pct
    if min_clearance > 0 and distance_pct < min_clearance:
        logger.info(
            "%s[L1] %s rejected — price %.2f only %.3f%% from VWAP %.2f "
            "(min_clearance=%.2f%% — AT RISK zone, stock hasn't established its side)",
            sym, direction_prelim, current_price, distance_pct * 100, vwap,
            min_clearance * 100,
        )
        return None
    ...
    logger.info(
        "[L1] Signal: %s  price=%.2f  vwap=%.2f  distance=%.3f%%  trend=%s",
        direction, current_price, vwap, distance_pct * 100, trend,
    )
    return EntrySignal(
        direction=direction,
...
def check_momentum_candle(bars_1m: list[Bar], direction: str) -> bool:
    """
    The last TWO completed 1-min bars must both close in the trade direction
    relative to their prior bar:
      CALL: bar1.close > bar2.close  AND  bar2.close > bar3.close  (2 rising closes)
      PUT:  bar1.close < bar2.close  AND  bar2.close < bar3.close  (2 falling closes)

    Candle color (close vs open) is intentionally NOT checked — it is intrabar
    noise that varies by exact tick timing, not a meaningful momentum signal.

    bars_1m[-2] = most recently COMPLETED bar   (bar1)
    bars_1m[-3] = bar before that               (bar2)
    bars_1m[-4] = reference close for bar2      (bar3)

    Two consecutive higher (or lower) closes confirms sustained directional
    momentum and filters single-bar micro-bounces within a declining sequence,
    while remaining achievable in normal VWAP pullback conditions.
    """
    if len(bars_1m) < 4:
        logger.debug("[L3] Not enough bars for 2-bar momentum check")
        return False

    bar1 = bars_1m[-2]   # most recently completed bar
    bar2 = bars_1m[-3]   # bar before that
    bar3 = bars_1m[-4]   # reference for bar2's prior close

    if direction == "CALL":
        ok = (bar1.close > bar2.close) and (bar2.close > bar3.close)
        if not ok:
            logger.info(
                "[L3] Momentum NOT confirmed for CALL — "
                "need 2 rising closes: bar1=%.2f bar2=%.2f bar3=%.2f "
                "(bar1>bar2=%s, bar2>bar3=%s)",
        ...
        if not ok:
            logger.info(
                "[L3] Momentum NOT confirmed for PUT — "
                "need 2 falling closes: bar1=%.2f bar2=%.2f bar3=%.2f "
                "(bar1<bar2=%s, bar2<bar3=%s)",
                bar1.close, bar2.close, bar3.close,
                bar1.close < bar2.close, bar2.close < bar3.close,
            )
