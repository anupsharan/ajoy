    ...
    configGroups: [
      ...
      // ── Structural levels + chop filter (shared S1 + S2) ─────────
      {
        ...
        fields: [
          ...
          { key: 'struct_min_stop_atr_mult',  label: 'Min Stop (× ATR)',        hint: 'Stop must sit at least this many 5-min ATRs from entry, else it is pushed out and size shrinks to keep dollar risk flat. Jul 30: all three losers had stops of 0.66–1.02× one candle. 0 = off', type: 'number', step: 0.1 },
          { key: 'struct_stop_atr_minutes',   label: 'Stop ATR Bar Size (min)', hint: 'Bar length the stop ATR is measured on (5 = 5-min candles)', type: 'number', step: 1 },
          { key: 'struct_stop_atr_period',    label: 'Stop ATR Period',         hint: 'How many of those bars the ATR averages (14 standard)', type: 'number', step: 1 },
          { key: 'struct_min_stop_pct',       label: 'Min Stop (% premium)',    hint: 'Option stop never tighter than this fraction of premium — floor against spread noise (0.08 = 8%). Blind to volatility; the ATR floor above is the real guard', type: 'number', step: 0.01 },
